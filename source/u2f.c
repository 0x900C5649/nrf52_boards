...
static int16_t
    u2f_authenticate(struct u2f_authenticate_request *req, uint8_t control);
#endif
    ...
    CTAP_RESPONSE *resp)
{
    ...
    if (rcode != U2F_SW_NO_ERROR
        && rcode
               != U2F_SW_CONDITIONS_NOT_SATISFIED)  // If the extension didn't do anything...
    {
#ifdef ENABLE_U2F
        switch (req->ins)
        {
            case U2F_REGISTER:
                ...
                else
                {
                    //                    timestamp();
                    rcode =
                        u2f_register((struct u2f_register_request *) payload);
...
{
    uint32_t len =
        ((req->LC3) | ((uint32_t) req->LC2 << 8) | ((uint32_t) req->LC1 << 16));
...
void u2f_reset_response() { ctap_response_init(_u2f_resp); }

void u2f_set_writeback_buffer(CTAP_RESPONSE *resp) { _u2f_resp = resp; }
...
static void
    u2f_make_auth_tag(struct u2f_key_handle *kh, uint8_t *appid, uint8_t *tag)
{
    uint8_t hashbuf[32];
    size_t  hashbuflen = sizeof(hashbuf);
    ...
    assert(hashbuflen >= CREDENTIAL_TAG_SIZE);
    memmove(tag, hashbuf, CREDENTIAL_TAG_SIZE);
}

int8_t
    u2f_new_keypair(struct u2f_key_handle *kh, uint8_t *appid, uint8_t *pubkey)
{
    crypto_generate_rng(kh->key, U2F_KEY_HANDLE_KEY_SIZE);
    u2f_make_auth_tag(kh, appid, kh->tag);

    crypto_ecc256_derive_public_key(
        (uint8_t *) kh,
        U2F_KEY_HANDLE_SIZE,
        pubkey,
        pubkey + 32);
...
int8_t u2f_authenticate_credential(
    ...
    u2f_authenticate(struct u2f_authenticate_request *req, uint8_t control)
{
    ...
    size_t   sigsize = sizeof(struct u2f_authenticate_request);
    ...
    if ((control != U2F_AUTHENTICATE_SIGN
         && control != U2F_AUTHENTICATE_SIGN_NO_USER)
...
static int16_t u2f_register(struct u2f_register_request *req)
{
    ...
    u2f_response_writeback(attestation_cert, attest_size);
