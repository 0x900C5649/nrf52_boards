...
static struct
{
    ...
    bool
        pending;  //!< Waiting for an fds FDS_EVT_DEL_RECORD event, to delete the next record.
...
static void fds_evt_handler(fds_evt_t const *p_evt)
{
    ...
    {
        NRF_LOG_INFO(
            "Event: %s (0x%X) received (NRF_SUCCESS)",
...
{
    while (!m_fds_initialized) { power_manage(); }
}

static void wait_for_fds_op(void)
{
    while (m_fds_ongoing_op) { power_manage(); }
}
...
uint8_t ctap_rk_size() { return MAX_KEYS; }
