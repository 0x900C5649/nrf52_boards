...
void crypto_sha256_update_secret()
{
    ...
    err_code = nrf_crypto_hash_update(&sha256_ctx, master_secret, 32);
    APP_ERROR_CHECK(err_code);
}
...
{
    err_code =
        nrf_crypto_hash_init(&sha512_ctx, &g_nrf_crypto_hash_sha512_info);
    APP_ERROR_CHECK(err_code);
...
{
    err_code = nrf_crypto_hmac_finalize(&hmac_ctx, hmac, len);
    APP_ERROR_CHECK(err_code);
}
    ...
    size_t * keylen)
{
    ...
    crypto_aes256_op(privkey, keylen);
...
void crypto_ecc256_compute_public_key(
    uint8_t *privkey,
    size_t   privlen,
    uint8_t *pubkey,
    size_t * publen)
...
void crypto_ecc256_make_key_pair(
    uint8_t *pubkey,
    size_t * publen,
    uint8_t *privkey,
    size_t * privlen)
...
{
    err_code =
        nrf_crypto_aes_init(&aes_ctx, &g_nrf_crypto_aes_cbc_256_info, op);
    APP_ERROR_CHECK(err_code);

    if (key == CRYPTO_TRANSPORT_KEY)
    {
        err_code = nrf_crypto_aes_key_set(&aes_ctx, transport_secret);
        APP_ERROR_CHECK(err_code);
        //AES_init_ctx(&aes_ctx, transport_secret);
    }
    else
    {
        err_code = nrf_crypto_aes_key_set(&aes_ctx, key);
        APP_ERROR_CHECK(err_code);
        //AES_init_ctx(&aes_ctx, key);
    }
    if (nonce == NULL)
    {
        err_code = nrf_crypto_aes_iv_set(&aes_ctx, NULLS);
        APP_ERROR_CHECK(err_code);
        //        memset(aes_ctx.Iv, 0, 16);
    }
    else
    {
        err_code = nrf_crypto_aes_iv_set(&aes_ctx, nonce);
        APP_ERROR_CHECK(err_code);
...
void crypto_aes256_op(uint8_t *buf, size_t *length)
{
    ...
    err_code = nrf_crypto_aes_finalize(&aes_ctx, buf, *length, buf, length);
