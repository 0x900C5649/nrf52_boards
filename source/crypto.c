...
void crypto_sha256_hmac_init(uint8_t * key, uint32_t klen)
{
    ...
    err_code = nrf_crypto_hmac_init(&hmac_ctx,
...
{
    err_code = nrf_crypto_hmac_update(&hmac_ctx,
                                        data,
                                        len
                                        );
    APP_ERROR_CHECK(err_code);
...
{
    err_code = nrf_crypto_hmac_finalize(&hmac_ctx,
                                        hmac,
                                        len
                                        );
    APP_ERROR_CHECK(err_code);
...
{
    err_code = nrf_crypto_aes_init( &aes_ctx,
    ...
    {
        err_code = nrf_crypto_aes_key_set(&aes_ctx,
                                            transport_secret
                                            );
        APP_ERROR_CHECK(err_code);
    ...
    if (nonce == NULL)
    {
        err_code = nrf_crypto_aes_iv_set(&aes_ctx,
                                            NULLS
                                            );
        APP_ERROR_CHECK(err_code);
