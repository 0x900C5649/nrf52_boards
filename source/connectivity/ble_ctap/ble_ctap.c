 ...
 */
static void ble_ctap_on_ctrl_pt_write(ble_ctap_t * p_ctap, ble_gatts_evt_write_t const * p_evt_write);
 ...
 * @param[in]   ble_gatts_evt_write_t   write event
 */
static void ble_ctap_on_revision_bitfield_write(ble_ctap_t * p_ctap, ble_gatts_evt_write_t const * p_evt_write);
 ...
 * @param[in]   p_ble_evt               ble event
 */
static void ble_ctap_on_disconnect(ble_ctap_t * p_ctap, ble_evt_t const * p_ble_evt);
 ...
 */
static ret_code_t ble_ctap_frame_send(uint16_t conn_handle,  ble_gatts_char_handles_t char_handle, BLE_CTAP_FRAME * p_frame, uint16_t * p_length, bool reqcmpl);
...
//    add_char_params.char_props.write_wo_resp = true;
    ...
    err_code = characteristic_add(p_ctap->service_handle,
                                    &add_char_params,
...
//    memcpy(&f, p_evt_write->data, p_evt_write->len);
    ...
    /**NRF_LOG_FLUSH();*/
        ...
        //alloc BLE_CTAP_REQ
        p_ctap->dispatchPackage = (BLE_CTAP_REQ *) nrf_malloc(sizeof(BLE_CTAP_REQ));
    ...
    NRF_LOG_DEBUG("checkpoint %d", __LINE__); 
        // check for ongoing fragment
        if (!p_ctap->waitingforContinuation || p_ctap->readyfordispatch)// && !p_ctap->readyfordispatch) {
        {
    NRF_LOG_DEBUG("[%d] waiting: %d, ready: %d", __LINE__, p_ctap->waitingforContinuation , p_ctap->readyfordispatch); 
    /**NRF_LOG_FLUSH();*/
            ble_ctap_send_err(p_ctap, BLE_CTAP_ERR_INVALID_SEQ);
            ble_ctap_clear_recv(p_ctap); // clean state
            return;
        ...
        if (p_ctap->recvnextseq != FRAME_SEQ(*f)){
            NRF_LOG_DEBUG("[%d] wrong seq %d, expected %d", __LINE__, FRAME_SEQ(*f), p_ctap->recvnextseq); 
            ble_ctap_send_err(p_ctap, BLE_CTAP_ERR_INVALID_SEQ);
            ble_ctap_clear_recv(p_ctap); // clean state
            return;
        }
        ...
        // move data
        tocpy = MIN(p_evt_write->len - 1, p_ctap->dispatchPackage->length - p_ctap->recvoffset);
        memcpy( ((void *)p_ctap->dispatchPackage->data) + p_ctap->recvoffset, f->cont.data, tocpy);
        NRF_LOG_DEBUG("checkpoint %d", __LINE__); 
    ...
    p_ctap->readyfordispatch = (p_ctap->recvoffset == p_ctap->dispatchPackage->length);
    NRF_LOG_DEBUG("checkpoint %d, readyfordispatch: %d", __LINE__, p_ctap->readyfordispatch); 
...
static void ble_ctap_clear_recv(ble_ctap_t * p_ctap)
{
    if(p_ctap->dispatchPackage != NULL) nrf_free((void *)p_ctap->dispatchPackage);
...
//    NRF_LOG_DEBUG("ble_ctap_on_ble_evt: 0x%x", p_ble_evt->header.evt_id);
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GATTS_EVT_WRITE:
            ble_ctap_on_write(p_ctap, p_ble_evt);
            break;
...
void ble_ctap_send_resp(ble_ctap_t * p_ctap, uint8_t cmd, uint8_t * p_data, size_t size, bool reqcomplete)
{
    ...
    size_t      eff_mtu = (size_t) nrf_ble_gatt_eff_mtu_get(p_ctap->m_gatt, p_ctap->conn_handle);
    ...
    do
    {
        ...
        // send frame
        err_code = ble_ctap_frame_send(p_ctap->conn_handle, p_ctap->status_handle, &frame, &payloadLen, reqcomplete);
        APP_ERROR_CHECK(err_code);
        m_hvx_pending = true;
...
bool ble_ctap_process(ble_ctap_t * p_ctap)
{
    ...
    if(p_ctap->readyfordispatch)// message to handle
    {
        ...
        p_ctap->request_handler(p_ctap,(BLE_CTAP_REQ *) p_ctap->dispatchPackage);
