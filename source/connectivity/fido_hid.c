 ...
 */
static uint8_t
    hid_if_send(uint32_t cid, uint8_t cmd, uint8_t *p_data, size_t size);
...
void hid_process(void) { internal_hid_process(); }
...
static void hid_init_response(hid_channel_t *p_ch)
{
    ...
    p_resp_init->capFlags     = CAPABILITY_WINK | CAPABILITY_CBOR
...
static void hid_cbor_response(hid_channel_t *p_ch)
{
    ...
    memcpy(
        p_ch->resp + 1,
        resp.data,
        MIN(resp.length, CTAP_RESPONSE_BUFFER_SIZE));
 ...
 * 
 */
static void hid_sync_response(hid_channel_t *p_ch) { return; }
...
static void channel_process(void)
{
    ...
    for (p_ch = m_ctap_ch_list.pFirst; p_ch != NULL;)
    {
        ...
        if (has_timer_expired(
                &p_ch->timer))  // && p_ch->state == CID_STATE_IDLE)
        {
            if (p_ch->cid != CID_BROADCAST)
            {
