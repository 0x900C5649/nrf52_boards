...
uint8_t ctap_make_credential(CborEncoder *encoder, uint8_t *request, int length)
{
    ...
    for (i = 0; i < MC.excludeListSize; i++)
    {
        ...
        dump_hex1(
            TAG_GREEN,
            (uint8_t *) &excl_cred->credential.id,
            sizeof(CredentialId));

        if (ctap_authenticate_credential(
                &MC.rp,
                excl_cred))  ///if credential authenticates -> key "exists" on this token
        {
...
uint8_t ctap_get_assertion(CborEncoder *encoder, uint8_t *request, int length)
{
    ...
    if (validCredCount < 2 || !getAssertionState.user_verified)
    {
        printf1(
            TAG_GREEN,
            "Only one account, nulling out user details on response\r\n");
    ...
    if (is_extension_request(
            (uint8_t *) &GA.creds[validCredCount - 1].credential.id,
            sizeof(CredentialId)))
    {
...
#endif
    {
        ...
        ((CTAP_authDataHeader *) auth_data_buf)->flags &= ~(1 << 2);
        ((CTAP_authDataHeader *) auth_data_buf)->flags |=
            (getAssertionState.user_verified << 2);

        {
            unsigned int ext_encoder_buf_size =
                sizeof(auth_data_buf) - auth_data_buf_sz;
    ...
    save_credential_list(
        (CTAP_authDataHeader *) auth_data_buf,
        GA.clientDataHash,
        GA.creds,
        validCredCount - 1);  // skip last one
...
uint8_t ctap_get_next_assertion(CborEncoder *encoder)
{
    ...
    memmove(
        &authData,
        &getAssertionState.authData,
        sizeof(CTAP_authDataHeader));
...
uint8_t ctap_request(uint8_t *pkt_raw, int length, CTAP_RESPONSE *resp)
{
    ...
    printf1(
        TAG_CTAP,
        "cbor output structure: %d bytes.  Return 0x%02x\n",
        resp->length,
        status);
...
void ctap_flush_state() { write_ctap_state(&STATE); }
    ...
    switch (desc->type)
    {
        case PUB_KEY_CRED_PUB_KEY:
            ...
            make_auth_tag(
                ...
                desc->credential.id.count,
                tag);
            return (
                memcmp(desc->credential.id.tag, tag, CREDENTIAL_TAG_SIZE) == 0);
            break;
        ...
        for (i = 0; i < STATE.rk_stored; i++)
        {
            ...
            if (memcmp(rk.id.rpIdHash, rpIdHash, 32) == 0)
            {
                ...
                if (count == ALLOW_LIST_MAX_SIZE - 1)
                {
                    printf2(
                        TAG_ERR,
                        "not enough ram allocated for matching RK's (%d).  "
                        "Skipping.\r\n",
                        count);
                    break;
                }
...
int8_t ctap_leftover_pin_attempts() { return STATE.remaining_tries; }
...
uint8_t ctap_is_pin_set() { return STATE.is_pin_set == 1; }
    ...
    uint8_t *pinHashEnc)
{
    ...
    if (memcmp(pinHashEncSalted, STATE.PIN_CODE_HASH, 16) != 0)
    {
        ...
        dump_hex1(TAG_ERR, KEY_AGREEMENT_PUB, ECC_PUBLIC_KEY_SIZE);
    ...
    CTAP_credInfo *credInfo)
{
    ...
    done_rk:
        ...
        auth_data_sz = sizeof(CTAP_authData)
 ...
 */
int ctap_encode_der_sig(
    const uint8_t *const in_sigbuf,
    uint8_t *const       out_sigder)
{
    ...
    memmove(
        out_sigder + 6 + 32 + pad_r + pad_s - lead_r,
        in_sigbuf + 32u + lead_s,
        32u - lead_s);
...
int8_t ctap_device_locked() { return STATE.remaining_tries <= 0; }

int8_t ctap_device_boot_locked() { return PIN_BOOT_ATTEMPTS_LEFT <= 0; }
    ...
    unsigned int *   ext_encoder_buf_size)
{
    ...
    if (ext->hmac_secret_present == EXT_HMAC_SECRET_PARSED)
    {
        ...
        crypto_sha256_hmac_init(shared_secret, 32);
        crypto_sha256_hmac_update(
            ext->hmac_secret.saltEnc,
...
uint8_t ctap_add_user_entity(CborEncoder *map, CTAP_userEntity *user)
{
    ...
    if (dispname)
    {
        ...
        ret =
            cbor_encode_text_stringz(&entity, (const char *) user->displayName);
        check_ret(ret);
    }
