...
#define CAPABILITY_WINK \
    0x01  // If set to 1, authenticator implements CTAPHID_WINK function
#define CAPABILITY_CBOR \
    0x04  // If set to 1, authenticator implements CTAPHID_CBOR function
#define CAPABILITY_NMSG \
    0x08  // If set to 1, authenticator DOES NOT implement CTAPHID_MSG function
    ...
    typedef struct __attribute__((__packed__))
    {
        ...
        uint32_t cid;                     // Channel identifier
        uint8_t
                versionInterface;  // Interface version // CTAPHID protocol version
 ...
 * */
#define ENDPOINT_LIST() (HID_EPIN, HID_EPOUT)
