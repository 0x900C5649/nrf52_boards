...
#define MIN_CONN_INTERVAL \
    MSEC_TO_UNITS(        \
        200,              \
        UNIT_1_25_MS) /**< Minimum acceptable connection interval (0.5 seconds). */
...
#define CONN_SUP_TIMEOUT \
    MSEC_TO_UNITS(       \
        5000,            \
        UNIT_10_MS) /**< Connection supervisory time-out (4 seconds). */


#define FIRST_CONN_PARAMS_UPDATE_DELAY \
    APP_TIMER_TICKS(                   \
        20000) /**< Time from initiating event (connect or start of notification) to first time sd_ble_gap_conn_param_update is called (15 seconds). */
